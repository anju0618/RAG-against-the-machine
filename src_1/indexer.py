...
class Indexer:
    ...
    def __init__(self, max_chunk_size: int = 2000) -> None:
        ...
        # 【修正】 data/raw/ 配下のディレクトリを動的に検索してコーパスパスを設定する
        # （評価環境でディレクトリ名が変更された場合への堅牢性向上）
        if raw_dir.exists():
            subdirs = [d for d in raw_dir.iterdir() if d.is_dir()]
            self.corpus_dir = subdirs[0] if subdirs else raw_dir / "vllm-0.10.1"
        else:
            self.corpus_dir = raw_dir / "vllm-0.10.1"
